...
For the Virtualization of KVM, the kvm guest information will be scanned for each hypervisor.
...
For the Virtualization of KVM, the kvm guest information will be updated if the kvm guest name and the hypervisor matched to the object which has been defined in the xCAT database.

Note: only the matched object will be updated.

B<-n>          Only for the Virtualization of KVM, write the kvm guest information to xCAT database which kvm guest name have not been defined in xCAT database.

B<-w>          Writes output to xCAT database.

For the Virtualization of KVM, output conflict information if the hypervisor not matched to the object which has been defined in the xCAT database with the same kvm guest name.
...
8. To list all kvm guest information managed by hypervisor hyp01 in stanza format and write the output to the xCAT database, enter:
