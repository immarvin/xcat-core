...
sub rscan {
    ...
    if ($@) {
        print STDERR "Unable to list all domains for $hyper" . $@->message . "\n";
    }
    my %host2kvm;
    my @displaymsg;
    if ($stanza) {
    ...
    my %handle_vmtab = ();
    $handle_vmtab{vm} = xCAT::Table->new( "vm", -create=>1, -autocommit=>0 );
    if (!$handle_vmtab{vm}) {
    ...
    my %hash_vm2host;
    my @vm_nodes_hosts = $handle_vmtab{vm}->getAllNodeAttribs(['node','host']);
    ...
    foreach $dom (@doms) {
        ...
        my $domain=$parser->parse_string($currxml);
        my @paths = $domain->findnodes("/domain");
        if (scalar @paths != 1) {
            xCAT::SvrUtils::sendmsg([1,"Can not find $name domain definition"], $callback,$hyper);
            next;
        }
        ...
        my @vmnicsinterfaceobjs = $domain->findnodes("/domain/devices/interface");
        foreach my $vmnicsinterfaceobj (@vmnicsinterfaceobjs) {
            if (($vmnicsinterfaceobj->getAttribute("type")) eq "bridge" ) {
        ...
        if ($write) {
            ...
            elsif ((exists $hash_vm2host{$node}) and ($hash_vm2host{$node} eq $hypervisor)) {
